...
class d_rep extends MY_Controller {
...
var $rows1;
	...
	$this->rows=$this->d_rep();
	$this->rows1=$this->d_rep();
	...
	if ($_REQUEST['generatereport'])
    {
		...
		if($_POST['facility']==0){
		        ...
		        if($_POST['period'] == 'Monthly'){
		              ...
		              $sql="SELECT Sample_ID AS sid,Patient_ID AS pid,lab_no AS a,End_Time AS b, fullname AS c,gender AS d,age AS e, pat_type AS f, mobile AS g, facilitys.name AS h,  Test_Result AS i,mtbRif AS rif,User as j 
							FROM sample1
		             ...
		             $sql="SELECT Sample_ID AS sid,Patient_ID AS pid,lab_no AS a,End_Time AS b, fullname AS c,gender AS d,age AS e, pat_type AS f, mobile AS g, facilitys.name AS h,  Test_Result AS i,mtbRif AS rif,User as j 
							...
							LEFT  JOIN `districts` ON `districts`.`ID` = `facilitys`.`district`
							LEFT  JOIN `countys` ON `countys`.`ID` = `districts`.`county`
							WHERE `countys`.`ID` ='$countyID' AND  End_Time BETWEEN '$startmonth' AND '$endmonth'  AND cond='1' ";
		             $sql1="SELECT count(CASE WHEN cond='1' THEN 1 ELSE 0 END ) as tests,sum( CASE WHEN Test_Result = 'positive' THEN 1 ELSE 0 END ) AS mtb,sum( CASE WHEN Test_Result =  'negative'  THEN 1 ELSE 0 END) as neg,sum( CASE WHEN mtbRif = 'positive' THEN 1 ELSE 0 END ) AS rif,sum( CASE WHEN Test_Result = 'ERROR' THEN 1 ELSE 0 END ) AS errs  
		                    FROM sample1
							LEFT  JOIN `facilitys` ON `sample1`.`facility` = `facilitys`.`facilitycode`
							LEFT  JOIN `districts` ON `districts`.`ID` = `facilitys`.`district`
							LEFT  JOIN `countys` ON `countys`.`ID` = `districts`.`county`
		             ...
		             $sql="SELECT Sample_ID AS sid,Patient_ID AS pid,lab_no AS a,End_Time AS b, fullname AS c,gender AS d,age AS e, pat_type AS f, mobile AS g, facilitys.name AS h,  Test_Result AS i,mtbRif AS rif,User as j 
							...
							LEFT  JOIN `districts` ON `districts`.`ID` = `facilitys`.`district`
							LEFT  JOIN `countys` ON `countys`.`ID` = `districts`.`county`
							WHERE `countys`.`ID` ='$countyID' AND  Year(End_Time) = '$yearly' AND cond='1'   ";
		             $sql1="SELECT count(CASE WHEN cond='1' THEN 1 ELSE 0 END ) as tests,sum( CASE WHEN Test_Result = 'positive' THEN 1 ELSE 0 END ) AS mtb,sum( CASE WHEN Test_Result =  'negative'  THEN 1 ELSE 0 END) as neg,sum( CASE WHEN mtbRif = 'positive' THEN 1 ELSE 0 END ) AS rif,sum( CASE WHEN Test_Result = 'ERROR' THEN 1 ELSE 0 END ) AS errs  FROM sample1
							LEFT  JOIN `facilitys` ON `sample1`.`facility` = `facilitys`.`facilitycode`
							LEFT  JOIN `districts` ON `districts`.`ID` = `facilitys`.`district`
							LEFT  JOIN `countys` ON `countys`.`ID` = `districts`.`county`
		             ...
		             $sql="SELECT Sample_ID AS sid,Patient_ID AS pid,lab_no AS a,End_Time AS b, fullname AS c,gender AS d,age AS e, pat_type AS f, mobile AS g, facilitys.name AS h,  Test_Result AS i,mtbRif AS rif,User as j 
							...
							LEFT  JOIN `districts` ON `districts`.`ID` = `facilitys`.`district`
							LEFT  JOIN `countys` ON `countys`.`ID` = `districts`.`county`
							WHERE `countys`.`ID` ='$countyID' AND End_Time BETWEEN '$startdate' AND '$enddate'  AND cond='1'   ";
		             $sql1="SELECT count(CASE WHEN cond='1' THEN 1 ELSE 0 END ) as tests,sum( CASE WHEN Test_Result = 'positive' THEN 1 ELSE 0 END ) AS mtb,sum( CASE WHEN Test_Result =  'negative'  THEN 1 ELSE 0 END) as neg,sum( CASE WHEN mtbRif = 'positive' THEN 1 ELSE 0 END ) AS rif,sum( CASE WHEN Test_Result = 'ERROR' THEN 1 ELSE 0 END ) AS errs  FROM sample1
							LEFT  JOIN `facilitys` ON `sample1`.`facility` = `facilitys`.`facilitycode`
							LEFT  JOIN `districts` ON `districts`.`ID` = `facilitys`.`district`
							LEFT  JOIN `countys` ON `countys`.`ID` = `districts`.`county`
		             ...
		             $sql="SELECT Sample_ID AS sid,Patient_ID AS pid,lab_no AS a,End_Time AS b, fullname AS c,gender AS d,age AS e, pat_type AS f, mobile AS g, facilitys.name AS h,  Test_Result AS i,mtbRif AS rif,User as j 
							...
							LEFT  JOIN `districts` ON `districts`.`ID` = `facilitys`.`district`
							LEFT  JOIN `countys` ON `countys`.`ID` = `districts`.`county`
							WHERE `countys`.`ID` ='$countyID' AND  End_Time ='$specificdate'  AND cond='1'   ";
		        	 $sql1="SELECT count(CASE WHEN cond='1' THEN 1 ELSE 0 END ) as tests, sum( CASE WHEN Test_Result = 'positive' THEN 1 ELSE 0 END ) AS mtb,sum( CASE WHEN Test_Result =  'negative'  THEN 1 ELSE 0 END) as neg,sum( CASE WHEN mtbRif = 'positive' THEN 1 ELSE 0 END ) AS rif,sum( CASE WHEN Test_Result = 'ERROR' THEN 1 ELSE 0 END ) AS errs 
		        	        FROM sample1
							LEFT  JOIN `facilitys` ON `sample1`.`facility` = `facilitys`.`facilitycode`
							LEFT  JOIN `districts` ON `districts`.`ID` = `facilitys`.`district`
     ...
     elseif ($_POST['facility']>0) {
     	 
     	
     	
     	if($_POST['period'] == 'Monthly'){
             
              $sql="SELECT Sample_ID AS sid,Patient_ID AS pid,lab_no AS a,End_Time AS b, fullname AS c,gender AS d,age AS e, pat_type AS f, mobile AS g, f.name AS h,  Test_Result AS i,mtbRif AS rif,User as j FROM   sample1 s , facilitys f WHERE s.Refacility = f.facilitycode AND month(End_Time) = '$monthly' AND Year(End_Time) = '$monthyear'   AND cond='1'  AND facility='$mfl'";
             ...
             $sql="SELECT Sample_ID AS sid,Patient_ID AS pid,lab_no AS a,End_Time AS b, fullname AS c,gender AS d,age AS e, pat_type AS f, mobile AS g, f.name AS h,  Test_Result AS i,mtbRif AS rif,User as j FROM   sample1 s , facilitys f WHERE s.Refacility = f.facilitycode AND End_Time BETWEEN '$startmonth' AND '$endmonth'  AND cond='1'  AND facility='$mfl'  ";
             ...
             $sql="SELECT Sample_ID AS sid,Patient_ID AS pid,lab_no AS a,End_Time AS b, fullname AS c,gender AS d,age AS e, pat_type AS f, mobile AS g, f.name AS h,  Test_Result AS i,mtbRif AS rif,User as j FROM   sample1 s , facilitys f WHERE s.Refacility = f.facilitycode AND Year(End_Time) = '$yearly' AND cond='1'  AND facility='$mfl'  ";
             ...
             $sql="SELECT Sample_ID AS sid,Patient_ID AS pid,lab_no AS a,End_Time AS b, fullname AS c,gender AS d,age AS e, pat_type AS f, mobile AS g, f.name AS h, Test_Result AS i,mtbRif AS rif,User as j FROM   sample1 s , facilitys f WHERE s.Refacility = f.facilitycode AND End_Time ='$specificdate'  AND cond='1'  AND facility='$mfl'  ";
