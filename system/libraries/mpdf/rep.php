<?php
	...
	//get yearly
	echo $yearly = $_POST['yearly'];
	$specificdate = $_POST['specificdate'];
	
        if($_POST['period'] == 'Monthly'){
             
              $sql="SELECT lab_no AS a,End_Time AS b, fullname AS c,gender AS d,age AS e, pat_type AS f, mobile AS g, f.name AS h,  Test_Result AS i,User as j FROM   sample1 s , facilitys f WHERE s.Refacility = f.facilitycode AND month(End_Time) = '$monthly' AND Year(End_Time) = '$monthyear'   AND cond='1'  AND facility='$mfl'    ";
             ...
             $sql="SELECT lab_no AS a,End_Time AS b, fullname AS c,gender AS d,age AS e, pat_type AS f, mobile AS g, f.name AS h,  Test_Result AS i,User as j FROM   sample1 s , facilitys f WHERE s.Refacility = f.facilitycode AND End_Time BETWEEN '$startmonth' AND '$endmonth'  AND cond='1'  AND facility='$mfl'  ";
        
		}elseif($_POST['period'] == 'Yearly'){
              
             $sql="SELECT lab_no AS a,End_Time AS b, fullname AS c,gender AS d,age AS e, pat_type AS f, mobile AS g, f.name AS h,  Test_Result AS i,User as j FROM   sample1 s , facilitys f WHERE s.Refacility = f.facilitycode AND Year(End_Time) = '$yearly' AND cond='1'  AND facility='$mfl'  ";
        
        }elseif($_POST['period'] == 'Date Range'){
              
             $sql="SELECT lab_no AS a,End_Time AS b, fullname AS c,gender AS d,age AS e, pat_type AS f, mobile AS g, f.name AS h,  Test_Result AS i,User as j FROM   sample1 s , facilitys f WHERE s.Refacility = f.facilitycode AND  End_Time BETWEEN '$startdate' AND '$enddate'  AND cond='1'  AND facility='$mfl'  ";
        
        }elseif($_POST['period'] == 'Specific Date'){
              
             $sql="SELECT lab_no AS a,End_Time AS b, fullname AS c,gender AS d,age AS e, pat_type AS f, mobile AS g, f.name AS h, Test_Result AS i,User as j FROM   sample1 s , facilitys f WHERE s.Refacility = f.facilitycode AND End_Time ='$specificdate'  AND cond='1'  AND facility='$mfl'  ";
